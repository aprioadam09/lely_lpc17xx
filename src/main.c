...
static int on_can_send(const struct can_msg *msg, uint_least8_t bus_id,
		void *data);
...
// Generated by `dcf2dev --no-strings lpc17xx.dcf lpc17xx_dev -o src/dev.c`
extern co_dev_t *lpc17xx_dev_init(void);

// The memory pool used for dynamic memory allocations.
...
main(void)
{
	...
	struct mempool pool;
	alloc_t *alloc = mempool_init(&pool, memory, sizeof(memory));

	// Initialize the CAN network interface.
	can_net_t *net = can_net_create(alloc, 0);
	assert(net);
...
static int
on_can_send(const struct can_msg *msg, uint_least8_t bus_id, void *data)
{
	(void)data;
	(void)bus_id;
...
on_dn_2000_00(co_sub_t *sub, struct co_sdo_req *req, co_unsigned32_t ac,
		void *data)
{
	assert(sub);
...
static co_unsigned32_t
on_up_2001_00(const co_sub_t *sub, struct co_sdo_req *req, co_unsigned32_t ac,
		void *data)
{
