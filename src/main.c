...
main(void)
{
	...
	for (;;) {
		...
		// ---- Logika Baca Tombol & Update Object Dictionary ----
		// Baca status pin P2.10. 'false' jika ditekan.
		bool current_button_state = Chip_GPIO_GetPinState(LPC_GPIO, 2, 10);

		// Cek apakah ada perubahan status tombol
		if (current_button_state != last_button_state) {
			last_button_state = current_button_state;

			// Konversi status pin (false=ditekan) ke nilai OD (1=ditekan)
			uint8_t od_value = current_button_state ? 0 : 1;

			// Cari sub-objek 0x2110 di OD
			co_sub_t *sub = co_dev_find_sub(dev, 0x2110, 0x00);
			if (sub) {
				// Perbarui nilainya di object dictionary
				co_sub_set_val_u8(sub, od_value);

				// PICU PENGIRIMAN TPDO (TAMBAHKAN INI)
